...
void help(void)
{
	...
	printf(" 6. Road sign (Y) detection\n\n");
...
extern int hazard_people, hazard_vehicle, hazard_sign;
...
int main( int argc, char* argv[] )
{
	...
	Mat im_frame,im_gray, im_lane_debug, hazard_icon, im_merge1, im_merge2, im_merge3, im_merge4;
	...
	Mat im_sign = Mat(ROWS, COLS, CV_8UC3);
	Mat im_sign_detect = Mat(ROWS, COLS, CV_8UC3);
	...
	while(1)
	{
		...
		switch(feature_num)									//turn feature on/off
		{
			...
			case 0: //all feature 
				...
				bitwise_or(im_merge3,im_hazard_detect,im_merge4);
				bitwise_or(im_merge4,im_sign_detect,im_final);
			...
			case 6: //sign detection
				im_sign = detect_sign(im_frame);					
				bitwise_or(im_sign,im_frame,im_sign_detect);
				if(all_feature == 0)
				{	
					cvNamedWindow("sign recognition",CV_WINDOW_NORMAL);
					imshow("sign recognition",im_sign_detect);					
					c = cvWaitKey(1);
					goto hazard_detect;
					break;	
				}
			...
			case 7: //hazard detection
				...
				if((hazard_lane == 1) || (hazard_people == 1) || (hazard_light == 1) || (hazard_vehicle == 1) || (hazard_sign == 1))
				{
					hazard_icon.copyTo(im_hazard(Rect(1100,200,hazard_icon.rows,hazard_icon.cols)));
					if(hazard_light == 1)
						putText(im_hazard, "T", cvPoint(1115, 195), FONT_HERSHEY_SIMPLEX, 0.6, cvScalar(255,255,255), 1, CV_AA);
					if(hazard_lane == 1)
						putText(im_hazard, "L", cvPoint(1130, 195), FONT_HERSHEY_SIMPLEX, 0.6, cvScalar(255,255,255), 1, CV_AA);
					if(hazard_people == 1)
						putText(im_hazard, "P", cvPoint(1145, 195), FONT_HERSHEY_SIMPLEX, 0.6, cvScalar(255,255,255), 1, CV_AA);
					if(hazard_vehicle == 1)
						putText(im_hazard, "V", cvPoint(1160, 195), FONT_HERSHEY_SIMPLEX, 0.6, cvScalar(255,255,255), 1, CV_AA);
					if(hazard_sign == 1)
						putText(im_hazard, "S", cvPoint(1175, 195), FONT_HERSHEY_SIMPLEX, 0.6, cvScalar(255,255,255), 1, CV_AA);

					hazard_lane = 0; hazard_vehicle = 0; hazard_light = 0; hazard_people = 0; hazard_sign = 0;
		...
		sprintf(filename, "./output/frame_%06d.jpeg", framecnt);
		imwrite(filename,im_final);
	}
